...
struct FFTRealOpLowering : public ConversionPattern {
  ...
  // takes in the pointer to the operation, list of operands, and the rewriter object
  // const - function doesn't modify the class it belongs to
  ...
  matchAndRewrite(Operation *op, ArrayRef<Value> operands,
    ...
    auto ub = rewriter.create<arith::ConstantIndexOp>(loc,
    tensorType.getShape()[0]); auto step =
    ...
    auto inputValue = rewriter.create<memref::LoadOp>(loc, input,
    ValueRange{iv}); auto constantZero =
    rewriter.create<arith::ConstantFloatOp>(loc, llvm::APFloat(0.0),
    rewriter.getF64Type()); rewriter.create<memref::StoreOp>(loc, inputValue,
    alloc_temp_real, ValueRange{iv}); rewriter.create<memref::StoreOp>(loc,
    constantZero, alloc_temp_imag, ValueRange{iv});
    rewriter.setInsertionPointAfter(load_temp);

    // alloc memory for reversed and dealloc when not required
    auto alloc_reversed_real = insertAllocAndDealloc(memrefType, loc,
    rewriter); auto alloc_reversed_imag = insertAllocAndDealloc(memrefType,
    loc, rewriter);

    // bit reversal constants
    auto constant1 = rewriter.create<arith::ConstantOp>(loc,
    rewriter.getI64IntegerAttr(1)); auto constant2 =
    ...
    auto i_val = rewriter.create<arith::IndexCastOp>(loc,
    ...
    auto reversed_i = rewriter.create<arith::IndexCastOp>(loc,
    rewriter.getIndexType(), rev);
    // Load values from temp arrays
    auto real_val = rewriter.create<memref::LoadOp>(loc, alloc_temp_real,
    ValueRange{i}); auto imag_val = rewriter.create<memref::LoadOp>(loc,
    alloc_temp_imag, ValueRange{i});
    // Store values in reversed arrays
    rewriter.create<memref::StoreOp>(loc, real_val, alloc_reversed_real,
    ValueRange{reversed_i}); rewriter.create<memref::StoreOp>(loc, imag_val,
    alloc_reversed_imag, ValueRange{reversed_i});
    ...
    auto pi = rewriter.create<arith::ConstantFloatOp>(loc,
    llvm::APFloat(M_PI), rewriter.getF64Type()); auto neg2 =
    rewriter.create<arith::ConstantFloatOp>(loc, llvm::APFloat(-2.0),
    rewriter.getF64Type());
    ...
    auto half_size = rewriter.create<arith::ShLIOp>(loc,
    rewriter.create<arith::ConstantIndexOp>(loc, 1), stage); auto full_size =
    rewriter.create<arith::ShLIOp>(loc, half_size,
    rewriter.create<arith::ConstantIndexOp>(loc, 1));
    ...
    auto odd_index = rewriter.create<arith::AddIOp>(loc, even_index,
    half_size);

    // Calculate twiddle factor
    auto j_i64 = rewriter.create<arith::IndexCastOp>(loc,
    rewriter.getI64Type(), j); auto j_f64 =
    rewriter.create<arith::SIToFPOp>(loc, rewriter.getF64Type(), j_i64); auto
    full_size_i64 = rewriter.create<arith::IndexCastOp>(loc,
    rewriter.getI64Type(), full_size); auto full_size_f64 =
    rewriter.create<arith::SIToFPOp>(loc, rewriter.getF64Type(),
    full_size_i64); auto angle_div = rewriter.create<arith::DivFOp>(loc,
    j_f64, full_size_f64); auto angle_mul =
    rewriter.create<arith::MulFOp>(loc, neg2, angle_div); auto angle_final =
    rewriter.create<arith::MulFOp>(loc, pi, angle_mul); auto cos =
    rewriter.create<math::CosOp>(loc, angle_final); auto sin =
    rewriter.create<math::SinOp>(loc, angle_final);

    // Load odd value
    auto odd_real = rewriter.create<memref::LoadOp>(loc, alloc_reversed_real,
    ValueRange{odd_index}); auto odd_imag =
    rewriter.create<memref::LoadOp>(loc, alloc_reversed_imag,
    ...
    auto t_real = rewriter.create<arith::SubFOp>(loc, odd_real_cos,
    odd_imag_sin);
    ...
    auto t_imag = rewriter.create<arith::AddFOp>(loc, odd_real_sin,
    odd_imag_cos);

    // Load even value
    auto even_real = rewriter.create<memref::LoadOp>(loc,
    alloc_reversed_real, ValueRange{even_index}); auto even_imag =
    rewriter.create<memref::LoadOp>(loc, alloc_reversed_imag,
    ValueRange{even_index});

    // Butterfly operation
    auto new_even_real = rewriter.create<arith::AddFOp>(loc, even_real,
    t_real); auto new_even_imag = rewriter.create<arith::AddFOp>(loc,
    even_imag, t_imag); auto new_odd_real =
    rewriter.create<arith::SubFOp>(loc, even_real, t_real); auto new_odd_imag
    = rewriter.create<arith::SubFOp>(loc, even_imag, t_imag);

    // Store results
    rewriter.create<memref::StoreOp>(loc, new_even_real, alloc_reversed_real,
    ValueRange{even_index}); rewriter.create<memref::StoreOp>(loc,
    new_even_imag, alloc_reversed_imag, ValueRange{even_index});
    rewriter.create<memref::StoreOp>(loc, new_odd_real, alloc_reversed_real,
    ValueRange{odd_index}); rewriter.create<memref::StoreOp>(loc,
    new_odd_imag, alloc_reversed_imag, ValueRange{odd_index});
...
void ToyToAffineLoweringPass::runOnOperation() {
  ...
  patterns.add<
      ...
      filterOpLowering, DivOpLowering, SumOpLowering, SinOpLowering,
      CosOpLowering, SquareOpLowering, FFT1DRealOpLowering, FFT1DImgOpLowering,
      SincOpLowering, GetElemAtIndxOpLowering, SetElemAtIndxOpLowering,
      ...
      FFT1DImgConjSymmOpLowering, FFTRealOpLowering, FFTImagOpLowering, MedianFilterOpLowering>(
      &getContext());
